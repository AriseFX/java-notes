...
![img_5.jpg](img/GDT表.jpg)
    ...
    图中段描述符的段基地址和段界限不是连续的，而是分成了好几段，非常不科学，这是为了兼容80286处理器，
    算是一个后遗症吧。
...
```cgo
    ...
    void do_no_page(unsigned long error_code , unsigned long address) {
        ...
        ...
        page = get_free_page(); //获取物理页
        bread_page(page, current->executable->i_dev, nr);//读块设备
        put_page(page, address); //物理页和地址建立映射（修改页表）
    }
...
由于篇幅问题这里只详细介绍最有效的（较少的缺页次数）。
    ...
    FIFO：先入先出。如果是常驻内存的数据呢，这块数据在内存中停留最久，但是最常使用，结果他们因为"先入”却要被换出了。
    
    MIN：淘汰将来最远使用的页，最优的方案。但没法预估将来的事情。

    LRU：用历史来预测将来，淘汰最近最长一段时间内没有使用过的页。前面提到过程序具有『局部性』，所以这种方案在实际的操作系统中最为有效。

LRU实现：
    
    1.很容易想到每个页维护一个时间戳，这样就能精准的判断要淘汰哪个页面。但每次访问物理页都要去维护时间戳，这是一个很大的开销！而且
    页的数量很大，每次查找最小的时间戳也是一个很大开销！这种方式不可行。
    2. 维护一个页码栈，每次将最近使用的页上浮，这样最少使用的页会在栈底，但是实现代价还是很大，每次地址访问都需要修改栈，涉及到多次
    拷贝。
    
    其实LRU算法是一个很理想化的算法，对于实际操作系统只能『近似实现』。纸上得来终觉浅啊！

下面介绍实际操作系统的LRU近似实现：
    
    将上面提到的时间戳换成0/1两种状态，将所有的页框维护成一个环形队列。
    那么当缺页发生时，如果扫描到的页的状态为0，就将该页换出，如果扫描到的页状态为1，那就使用非常接地气的
    『再给一次机会』算法，把页状态置成0，那么这个页下次扫描到才会被换出。当使用该页的时候将其状态置1。
    本质上还是LRU，只不过把『最近最少使用』近似为『最近没有使用』。如下图：
        

![img_5.jpg](img/LRU1.jpg)

    上面提到的置换算法也叫Clock算法，但是上面对LRU实现的还是不够精准。
    如果缺页极少发生呢，会发生什么？所有的页状态都是1，这种情况下一旦发生缺页，指针会scan一圈，回到原点的时候
    发现这个页的状态还是0（转一圈的时间内还是没有被使用），OK，把这页置换出去。如果一直都是极少发生缺页呢（缺页一段时间后，
    所有页都被使用过了，都置成了1），那就变成了一直淘汰下一个，退化成了FIFO。如下图：

![img_5.jpg](img/LRU2.jpg)

    归根结底还是状态为1的页太多，导致每次scan花的时间太长，使LRU失去了『最近』，那么需要改进为『CLOCK』算法，
    再加一个快指针定时将1置成0，原来的慢指针用来scan没有使用的页，有了快指针，『最近』就有了，这是一个较为完善的LRU实现。
    如下图：
![img_5.jpg](img/LRU3.jpg)
